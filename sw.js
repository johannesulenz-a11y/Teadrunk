const CACHE_NAME = 'teadrunk-cache-v2.0';
...
self.addEventListener('fetch', e => {
  ...
  if (isIndex) {
    e.respondWith(fetch(new Request(e.request,{cache:'no-store'})).then(r => { if(r.ok) caches.open(CACHE_NAME).then(c=>c.put('./index.html',r.clone())); return r; }).catch(() => caches.match('./index.html')));
    return;
  }
